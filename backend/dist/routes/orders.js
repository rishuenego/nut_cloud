...
router.post('/', isAuthenticated, async (req, res) => {
    try {
        ...
        try {
            ...
            if (!isNaN(settingsMap['free_shipping_threshold']))
                freeShippingThreshold = settingsMap['free_shipping_threshold'];
            if (!isNaN(settingsMap['shipping_charge']))
                shippingChargeAmount = settingsMap['shipping_charge'];
            if (!isNaN(settingsMap['cod_charge']))
                codChargeAmount = settingsMap['cod_charge'];
        }
        catch (err) {
            ...
            try {
                ...
                if (coupon) {
                    ...
                    if (now >= new Date(coupon.valid_from) &&
                        ...
                        discountAmount = Math.min((subtotal * coupon.discount_percentage) / 100, Number(coupon.max_discount_amount));
...
router.get('/', isAuthenticated, async (req, res) => {
    try {
        const authReq = req;
        const userId = authReq.user.id;
        const orders = await getMany('SELECT * FROM orders WHERE user_id = ? AND (payment_method = "cod" OR payment_status = "paid") ORDER BY created_at DESC', [userId]);
        res.json({
            success: true,
...
router.get('/:id', isAuthenticated, async (req, res) => {
    try {
        ...
        const authReq = req;
        const userId = authReq.user.id;
        const order = await getOne('SELECT * FROM orders WHERE id = ? AND user_id = ? AND (payment_method = "cod" OR payment_status = "paid")', [id, userId]);
        if (!order) {
